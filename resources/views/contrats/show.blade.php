	...
	<div class="container">
		...
		<div class="row">
			<div class="col-lg-12 col-md-12 col-sm-12">
				...
				<hr>
				@if($contrat->client->type == 1)
					<div class="ttl-amts">
						@if ( ($contrat->date_retour_reelle)->format('Y-m-d') == '1000-11-23' )

						<h5>  Tax (9.5%) : {{ number_format( 0.095 * ($contrat->voiture->prix - $contrat->remise) * (($contrat->created_at)->startOfDay())->diffInDays(($contrat->date_retour_prevue)->startOfDay()), 0) }} F Cfa</h5>
						@else
						@if ( (($contrat->created_at)->startOfDay())->diffInDays(($contrat->date_retour_reelle)->startOfDay()) == 0)
						<h5>  Tax (9.5%) : {{ number_format( 0.095 * ($contrat->voiture->prix - $contrat->remise), 0) }} F Cfa</h5>
						@else
						<h5>  Tax (9.5%) : {{ number_format( 0.095 * ($contrat->voiture->prix - $contrat->remise) * (($contrat->created_at)->startOfDay())->diffInDays(($contrat->date_retour_reelle)->startOfDay()), 0) }} F Cfa</h5>
						@endif
						@endif
					</div>
					<hr />
					<div class="ttl-amts">
						...
						<h5>  Montant Net : {{ number_format( 0.905 * ($contrat->voiture->prix - $contrat->remise) * (($contrat->created_at)->startOfDay())->diffInDays(($contrat->date_retour_prevue)->startOfDay()), 0) }} F Cfa</h5>
						@else
						@if ( (($contrat->created_at)->startOfDay())->diffInDays(($contrat->date_retour_reelle)->startOfDay()) == 0)
						<h5>  Montant Net : {{ number_format( 0.905 * ($contrat->voiture->prix - $contrat->remise), 0) }} F Cfa</h5>
						@else
						<h5>  Montant Net : {{ number_format( 0.905 * ($contrat->voiture->prix - $contrat->remise) * (($contrat->created_at)->startOfDay())->diffInDays(($contrat->date_retour_reelle)->startOfDay()), 0) }} F Cfa</h5>
						@endif
						@endif
					</div>
				@endif
